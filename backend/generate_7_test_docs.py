...
doc2.add_paragraph('FR-108: The checkout service shall validate maximum allowable loan quota per member before approving checkout.')
...
doc3.add_paragraph('FS-208: Quota validation check rejects checkout request if member has reached 5 active loans.')
doc3.add_paragraph('FS-209: Audit interceptor persists actor ID, timestamp, and action payload to PostgreSQL audit log.')
doc3.add_paragraph('FS-211: The service shall store passwords using reversible encryption so administrators can recover the original password.') # INTENTIONAL SECURITY CONFLICT with FR-105
...
doc4.add_paragraph('US-308: As a Member, I want to view my active loan count so that I do not exceed my loan limit quota.')
...
doc4.add_paragraph('US-310: As a Member, I want to order coffee from the library cafeteria kiosk.') # Intentionally unmapped
doc4.add_paragraph('US-311: As a User, I want to sign in with social OAuth providers.') # Partial mapping
...
doc5.add_paragraph('TC-407: Test Scenario: Book Loan Renewal. Verify loan renewal extends due date by 14 days when no hold exists.')
...
doc5.add_paragraph('TC-409: Test Scenario: Loan Quota Enforcement. Verify checkout fails when active loan count equals 5 items.')
...
doc6 = Document()
doc6.add_heading('Change Request - Engineering Change CR-501', 0)
doc6.add_paragraph('Change Request: CR-501')
doc6.add_paragraph('Requested Change: Enforce Multi-Factor Authentication (MFA) via TOTP for all Librarian and Admin logins.')
doc6.add_paragraph('Reason for change: Security compliance and access control hardening.')
doc6.add_paragraph('Change impact: Impacts authentication module FR-105 and FS-205 access validation.')
...
doc7.add_paragraph('Meeting Minutes MOM-601: Date August 28, 2026')
doc7.add_paragraph('Attendees: Chief Architect, Lead Backend Engineer, QA Lead, Product Owner.')
doc7.add_paragraph('Agenda: Review sprint 14 deliverables, authentication architecture, and test coverage matrix.')
doc7.add_paragraph('Discussion: Discussed CR-501 MFA requirements and catalog search indexing.')
doc7.add_paragraph('Decisions: Approved CR-501 for Sprint 14 implementation. Confirmed TC-401 through TC-410 QA suite.')
